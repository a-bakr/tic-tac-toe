...
class Player:
    ...
    def load_policy(self):
        with open(f'{base_dir}/policy_%s.bin' % ('first' if self.symbol == 1 else 'second'), 'rb') as f:
            print(f'Loading policy from {base_dir}/policy_%s.bin' % ('first' if self.symbol == 1 else 'second'))
...
class HumanPlayer:
    ...
    def act(self):
        if self.i is not None and self.j is not None:
            return self.i, self.j, self.symbol
        
        self.state.print_state()
        key = input("Input your position:")
        data = self.keys.index(key)
        self.i = data // BOARD_COLS
        self.j = data % BOARD_COLS
        return self.i, self.j, self.symbol
...
def compete(turns):
    ...
    for _ in range(turns):
        ...
        if winner == -1:
            player2_win += 1
        judger.reset()
    print('%d turns, player 1 win %.02f, player 2 win %.02f' % (turns, player1_win /
                                                                turns, player2_win / turns))
